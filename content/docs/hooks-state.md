...
We hebben ook wat meer geleerd over wat Hooks zijn. Hooks zijn functies die je laten "aanhaken" (of "hook into") React voorzieningen vanuit functie componenten. Hun namen beginnen altijd met `use`, en er zijn meer Hooks die we nog niet gezien hebben.

**Laten we nu doorgaan met [het leren van de volgende Hook: `useEffect`.](/docs/hooks-effect.html)** Het laat je neven effecten uitvoeren in componenten, en lijkt op lifecycle methoden in classes.
