...
## User Experience Voorrang Geven {#putting-user-experience-first}
...
## Best Practices voor Suspense {#best-practices-for-suspense}
...
## Rendereren Terwijl je Laad {#render-as-you-fetch}
...
### Parallelle Data en View Trees {#parallel-data-and-view-trees}
...
### Laden in Event Handlers {#fetch-in-event-handlers}
...
### Laad Data Incrementeel {#load-data-incrementally}
...
### Behandel Code als Data {#treat-code-like-data}
...
## Deze Patronen Gebruiken {#using-these-patterns}
...
* We hebben twee vergelijkbare voorbeeld-apps gemaakt die deze concepten demonstreren:
  * De [Relay Hooks voorbeeld-app](https://github.com/relayjs/relay-examples/tree/master/issue-tracker) gebruikt GitHub's publieke GraphQL API om een eenvoudige issue-tracker-app te implementeren. Het bevat geneste route ondersteuning met het vooraf laden van code en data. De code is volledig The code is fully becommentarieerd -- we raden aan om deze repo the clonen en de app lokaal uit te voeren, en te onderzoeken hoe hij werkt.
  * We hebben ook een [non-GraphQL versie van de app](https://github.com/gaearon/suspense-experimental-github-demo) die demonstreert hoe deze concepten toegepast kunnen worden in andere data-fetch-blibliotheken.

Hoewel de APIs rond de Concurrent Mode en Suspense [nog steeds experimenteel](/docs/concurrent-mode-adoption.html#who-is-this-experimental-release-for) zijn, zijn we ervan overtuigd, dat de ideeën in dit artikel bewezen zijn door de praktijk. We begrijpen echter dat Relay en GraphQL niet voor iedereen geschikt zijn. Dat is prima! **We onderzoeken actief hoe we deze patronen kunnen generaliseren naar benaderingen zoals REST,** en onderzoeken ideeën voor een meer generieke (dus non-GraphQL) API voor het samenstellen van een boom van data-afhankelijkheden. Ondertussen kijken we reikhalzend uit welke nieuwe blibliotheken er te voorschijn zullen komen die de patronen die in dit artikel beschreven staan implementeren om het makkelijker te maken geweldige, *snelle* user-experiences te bouwen.
